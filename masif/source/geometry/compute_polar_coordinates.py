...
def compute_polar_coordinates(mesh, do_fast=True, radius=12, max_vertices=200):
    ...
    for i in range(n): 
        ...
        # --- Fix: force all .todense() to np.asarray
        rho_out[i,:len(neigh)]= np.squeeze(np.asarray(D[i,neigh].todense()))
        theta_out[i,:len(neigh)]= np.squeeze(np.asarray(theta[i][neigh]))
...
def compute_thetas(plane, vix, verts, faces, normal, neighbors, idx):
    ...
    """
    # --- Fix: ensure plane is np.ndarray, not np.matrix
...
def call_mds(mds_obj, pair_dist):
    # --- Fix: ensure input to fit_transform is nd-array
...
def compute_theta_all(D, vertices, faces, normals, idx, radius):
    ...
    for i in range(D.shape[0]):
        ...
        # --- Fix: .todense() returns matrix, convert to array
        pair_dist_i = np.asarray(pair_dist_i.todense())
        ...
        # --- Fix: ensure theta is ndarray (for possible legacy code)
...
def compute_theta_all_fast(D, vertices, faces, normals, idx, radius):
    ...
    for i in range(D.shape[0]):
        ...
        # --- Fix: .todense() returns matrix, convert to array
        dist_kk = np.asarray(dist_kk.todense())
