...
def generate_surface_fingerprints(precomputed_features_dir, output_dir, ppi_pair_id, custom_params_file=None):  
    ...
    if not os.path.exists(model_path + ".meta"): # TensorFlow adds .meta extension  
        print(f"错误: 模型文件未找到。请确保模型位于 {params['model_dir']}。")  
        print("尝试从 masif/data/masif_ppi_search/nn_models/sc05/all_feat/model_data/ 复制模型文件。")  
        return None  
...
if __name__ == "__main__":  
    # 假设Step 3的输出目录是 './output/precomputed/complex_A/'  
    precomputed_dir = "/xcfhome/zncao02/dataset_bap/test_set/custom/2wh8-II2/output/precomputed/complex_A/"  
    final_output_dir = "/xcfhome/zncao02/dataset_bap/test_set/custom/2wh8-II2/output"  
    protein_ligand_id = "complex_A"  
      
    # 默认参数文件路径 (需要根据你的实际安装路径调整)  
    # 假设 masif/data/masif_ppi_search/nn_models/sc05/all_feat/custom_params.py 存在  
    # 这里的路径是相对于 PYTHONPATH 的模块路径  
    custom_params_module_path = "masif_ppi_search.nn_models.sc05.all_feat.custom_params"  
  
    fingerprints = generate_surface_fingerprints(  
        precomputed_features_dir=precomputed_dir,  
        output_dir=final_output_dir,  
        ppi_pair_id=protein_ligand_id,  
        custom_params_file=custom_params_module_path  
    )  
